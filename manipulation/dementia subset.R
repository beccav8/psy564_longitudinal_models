...
dementia_data <- data[which(data$dementia==1),]
length(unique(dementia_data$projid))
#472 unique id's 
...
names_old <- as.data.frame(names(dementia_data)) 
...
names(dementia_data) <- replaced[,"name_new"]
names(dementia_data)

# ---- assing-variable-labels --------------------------------
for(name_ in names(dementia_data)){
  attr(dementia_data[,name_], "label") <- meta[meta$name_new == name_, "label"]
}

# ---- inspect-dementia_data ----------------------------------------------
# inspect loaded dementia_data objects (using basic demographic variables )
length(unique(dementia_data$id)) # there are this many of subjects 
# t <- table(dementia_data[,"fu_year"], dementia_data[,"died"]); t[t==0]<-".";t
t <- table(dementia_data[,"msex"], dementia_data[,"race"], useNA = "always"); t[t==0]<-".";t
t <- table(dementia_data[,"edu"], dementia_data[,"race"]); t[t==0]<-".";t

# ---- assemble-dementia_data-object-dto-1 --------------------------------------
dto <- list()
# the first element of dementia_data transfer object contains unit data
dto[["unitData"]] <- dementia_data
# the second element of dementia_data transfer object contains meta dementia_data
dto[["metaData"]] <-  meta
# verify and glimpse
dto[["unitData"]] %>% dplyr::glimpse()
dto[["metaData"]] %>% dplyr::glimpse()
# names_labels(dementia_data) %>% head()
# ---- save-to-disk ------------------------------------------------------------
...
saveRDS(dto, file="./data/unshared/derived/map2016/dto_raw_dementia_only.rds", compress="xz")

# ---- object-verification ------------------------------------------------
# the production of the dto object is now complete
# we verify its structure and content:
dto <- readRDS("./data/unshared/derived/map2016/dto_raw_dementia_only.rds")
names(dto)
