...
g5<- ggplot2::ggplot(ds0, aes_string(x= "year_in_study", y="sdmt", linetype="Sex")) +
...
g1<- ggplot2::ggplot(ds0, aes_string(x= "phys_pmeanC", y="sdmt")) +
  geom_point(shape=10, size=1)+
  stat_smooth(method=lm, se=TRUE)+
  theme1
g1 <- g1 + labs(list(
...
g1<- ggplot2::ggplot(ds0, aes_string(x= "pss_pmeanC", y="sdmt")) +
  geom_point(shape=10, size=1)+
  stat_smooth(method=lm, se=TRUE)+
  theme1
g1 <- g1 + labs(list(
...
#age_bl_gmc = y01 - when time =0, for every increase in BL age, sdmt score decreases this much 
...
# 0.32204/ (sqrt( 10492))
 ...
 # gender X PA
 eq5s <- as.formula("sdmt ~ 1 + year_in_study*age_bl_gmc + year_in_study*msex  + year_in_study*edu +
...
#  
 ...
 eq6s <- as.formula("sdmt ~ 1 + year_in_study*age_bl_gmc + year_in_study*msex  + year_in_study*edu +
                  pss_pmeanC*msex + pss_wp*msex +
                    ( 1 + year_in_study + pss_wp |id)")
 model_6s<- lmerTest::lmer(eq6s, data=ds0, REML= FALSE) 
 lmerTest::summary((model_6s))
...
#Physical Activity --------------
 eq7 <- as.formula("sdmt ~ 1 + year_in_study*age_bl_gmc + year_in_study*msex  +  year_in_study*edu +
                  phys_pmeanC*pss_pmeanC  + pss_pmeanC*phys_wp +
                  ( 1 + year_in_study + phys_wp |id)")
