...
#episodic
indwm<- ggplot(graph_sample, aes(x= year_in_study, y= episodic)) +geom_point()
...
g1<- ggplot2::ggplot(ds0, aes_string(x= "year_in_study", y="episodic", linetype="Sex")) +
...
#WM
eq <- as.formula("episodic ~ 1 + year_in_study +          
                 ( 1 + year_in_study |id)")
...
eq2 <- as.formula("episodic ~ 1 + year_in_study*age_bl_gmc + 
                  ( 1 + year_in_study |id)")
model_2<- lmerTest::lmer(eq2, data=ds0, REML= FALSE) 
lmerTest::summary((model_2))

#int = y00 -  mean WM score when age_bl = 0 *i.e. the mean
#age_bl_gmc = y01 - when time =0, for every increase in BL age, episodic score decreases this much 
...
eq3 <- as.formula("episodic ~ 1 + year_in_study*age_bl_gmc + year_in_study*msex  + 
                  ( 1 + year_in_study |id)")
...
eq4 <- as.formula("episodic ~ 1 + year_in_study*age_bl_gmc + year_in_study*msex  + year_in_study*edu + 
                  ( 1 + year_in_study |id)")
...
#Physical Activity --------------
eq5 <- as.formula("episodic ~ 1 + year_in_study*age_bl_gmc + year_in_study*msex  + year_in_study*edu +
                  phys_pmeanC + phys_wp +
                  ( 1 + year_in_study |id)")
...
# # gender X PA 
# eq6 <- as.formula("episodic ~ 1 + year_in_study*age_bl_gmc + year_in_study*msex  + year_in_study*edu +
...
#Physical Activity --------------
eq7 <- as.formula("episodic ~ 1 + year_in_study*age_bl_gmc + year_in_study*msex  +  year_in_study*edu +
