...
# @knitr declare-globals ---------------------------------------------------------
path_input2  <- "./data/unshared/derived/map/dwn_map_mmse.rds"
path_input1<- "./data/unshared/derived/map/dbio_map_mmse.rds"
...
for (i in 1: length(d$phys_wp)) {
    ...
    if (isTRUE(d$phys_wp[i] > 0.77 )) {
      d$phys_wp_dichot[i] <- "HIGH" }
    
    else if (isTRUE(d$phys_wp[i] < (-1.1 ))){
...
# eq_a <- as.formula("mmse_wp ~ 1 + full_year + age_bl_centered + full_year:age_bl_centered + edu + phys_bp_median  + phys_wp + 
#                      (1 + full_year |id)")
# # @knitr mmse_wp_full ---------------------------------
