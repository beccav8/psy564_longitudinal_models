...
path_input2  <- "./data/unshared/derived/obas/dwn_obas_dsb.rds"
path_input1<- "./data/unshared/derived/obas/dbio_obas_dsb.rds"
...
gdsb<- ggplot2::ggplot(dtest, aes_string(x= "years_in_study", y="dsb")) +
...
gdsb <- gdsb + labs(list(
  ...
  x="time", y="dsb"))
gdsb
#dsb declines over time, but there is much more variabliltiy in the middle of the study time
#low scores in mid
...
###############dsb versus PA
#raw
gPA<- ggplot2::ggplot(dtest, aes_string(x= "walk_now", y="dsb")) +
  geom_point(shape=21, size=5)+
  stat_smooth(method=lm, se=TRUE)+
  scale_color_brewer(palette="Set2") +
  coord_cartesian(ylim = c(0,30)) +
...
#bp
gPA_bp<- ggplot2::ggplot(dtest, aes_string(x= "phys_bp_median", y="dsb")) +
  geom_point(shape=21, size=5)+
  stat_smooth(method=lm, se=TRUE)+
  scale_color_brewer(palette="Set2") +
  coord_cartesian(ylim = c(0,30)) +
...
gPA_wp<- ggplot2::ggplot(dtest, aes_string(x= "phys_wp", y="dsb")) +
  geom_point(shape=21, size=5)+
  stat_smooth(method=lm, se=TRUE)+
  scale_color_brewer(palette="Set2") +
  coord_cartesian(ylim = c(0,30)) +
...
gPA_wp
#walking more than average is associated with higher dsb scores (also likely because they walk more when they are younger, and age brings their mean down)
...
p1 <- ggplot(d, aes(x=phys_wp, y=dsb, group=id)) +
  geom_line() +
