...
# knitr::stitch_rmd(
#   script="./sandbox/map/building-models/3-dsb-map-LH.R",
#   output="./sandbox/map/stitched-output/3-dsb-map-LH.md"
# )
# 
...
path_input1 <-  "./data/unshared/derived/obas/dwn_obas_dsb.rds" 
path_input2 <- "./data/unshared/derived/obas/dbio_obas_dsb.rds"
...
for (i in 1: length(dtemp$phys_wp)) {
    ...
    if (isTRUE(dtemp$phys_wp[i] > 2.39 )) {
      dtemp$phys_wp_dichot[i] <- "HIGH" }
    
    else if (isTRUE(dtemp$phys_wp[i] < (-2.98 ))){
...
dtemp %>%
 ...
 g<- ggplot2::ggplot(dtemp, aes_string(x= "years_in_study", y="pred", colour="phys_wp_dichot")) +
  geom_point(shape=21, size=5)+
  stat_smooth(method=lm, se=FALSE)+
  main_theme
# stat_smooth(method=lm, level=0.95, fullrange=FALSE)
g <- g + labs(list(
  title="interaction between phy_wp and time in study for females",
