...
read_chunk("./sandbox/obas/building-models/dsb-obas-LH.R") 
```
...
```{r tvc-intxn-equation-age, echo=C_echo, message=C_message, warning=C_warning, eval=FALSE}
```

```{r tvc-intxn, echo=C_echo, message=C_message, warning=C_warning, eval=FALSE}
```

```{r tvc-intxn-graph, echo=FALSE, message=C_message, warning=C_warning, eval=FALSE}
```
...
```{r dsb_wp_fluctuations, echo=C_echo, message=C_message, warning=C_warning}
```

```{r dsb_wp_full, echo=C_echo, message=C_message, warning=C_warning}
```

```{r dsb_wp_male, echo=C_echo, message=C_message, warning=C_warning}
```

```{r dsb_wp_female, echo=C_echo, message=C_message, warning=C_warning}
```
