...
# knitr::stitch_rmd(
#   script="./sandbox/map/building-models/3-mmse-map-LH.R",
#   output="./sandbox/map/stitched-output/3-mmse-map-LH.md"
# )
# 
...
path_input1 <-  "./data/unshared/derived/obas/dwn_obas_mmse.rds" 
path_input2 <- "./data/unshared/derived/obas/dbio_obas_mmse.rds"
  
# @knitr load-data ---------------------------------------------------------------
ds0  <- readRDS(path_input0) #total raw data
dwn  <- readRDS(path_input1) #subset for models
dbio <- readRDS(path_input2) #subset for biomarker models
...
for (i in 1: length(d$phys_wp)) {
  
  if (isTRUE(d$phys_wp[i] > 2.39 )) {
    d$phys_wp_dichot[i] <- "HIGH" }
  
  else if (isTRUE(d$phys_wp[i] < (-2.98 ))){
    d$phys_wp_dichot[i] <- "LOW"  }
  
  else {
    d$phys_wp_dichot[i] <- "mid"
  }
...
d %>%
 ...
 g<- ggplot2::ggplot(d, aes_string(x= "years_in_study", y="pred", colour="phys_wp_dichot")) +
  geom_point(shape=21, size=5)+
  stat_smooth(method=lm, se=FALSE)+
  main_theme
# stat_smooth(method=lm, level=0.95, fullrange=FALSE)
g <- g + labs(list(
  title="interaction between phy_wp and time in study",
...
g<- ggplot2::ggplot(d, aes_string(x= "age_at_visit_centered", y="pred", colour="phys_wp_dichot")) +
  geom_point(shape=21, size=5)+
  stat_smooth(method=lm, se=FALSE)+
  main_theme
# stat_smooth(method=lm, level=0.95, fullrange=FALSE)
...
#when you fluctuate to be higher than usual, mmse score is relatively stable
#when you are lower than usual in PA, mmse score declines at that occasion, over time, the effects of
#being lower than usual in PA result in lower mmse scores 
#- it is important to keep exercising into old age?


# @knitr mmse_wp_fluctuations ---------------------------------

###using mmse deviation from mean as outcome  
### thus, those who score higher than their average on physical activity, 
###also score higher than their average mmse 

eq_a <- as.formula("mmse_wp ~ 1 + years_in_study + age_bl_centered + years_in_study:age_bl_centered + edu + phys_bp_median  + phys_wp + 
                     (1 + years_in_study |id)")
# @knitr mmse_wp_full ---------------------------------
...
# g<- ggplot2::ggplot(cf, aes_string(x="years_in_study.phys", y="X.Intercept.")) +
